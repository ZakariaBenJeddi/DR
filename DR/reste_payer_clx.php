...
df.value = today;
                ...
                <div class="card-body mt-2 " >
                  ...
                  <table id="example1" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?php
$query = mysqli_query($con, "SELECT * FROM demande_prix_complex WHERE DATE_FORMAT(dated,'%Y-%m') >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 12 MONTH),'%Y-%m') and idcomplex=(select idcomplex from complexe1 where id=$_SESSION[sid]) and deviis=0 and payer=0");
if(isset($_POST['cht'])){
                        $query=mysqli_query($con,"SELECT * FROM demande_prix_complex where idcomplex=(select idcomplex from complexe1 where id=$_SESSION[sid]) and deviis=0 and payer=0");
                      }
                      if(isset($_POST['chercher'])){
                        $dd=$_POST['dd'];
                        $ff=$_POST['df'];
                        $query=mysqli_query($con,"SELECT * FROM demande_prix_complex WHERE dated between '$dd' AND '$ff' and idcomplex=(select idcomplex from complexe1 where id=$_SESSION[sid]) and deviis=0 and payer=0");    
                }
                      $cnt=1;
...
echo $res; // This will output the remaining days in the 60-day period
                          } 
                          if($row['bcmd']==1){
                            echo "---";
                          }
...
filteredData.each(function (valueArray) {
    ...
    valueArray.forEach(function (value,index) {
         ...
         if (index !=0 && index !=2 && index !=8 && index !=4) {
...
workbook.xlsx.writeBuffer().then(function (buffer) {
    ...
    a.href = url;
    a.download = 'Bcmd/factures non payés.xlsx';
