...
if(isset($_GET['dev'])) {
  $id = intval($_GET['id']);
  $stmt = $dbh->prepare("UPDATE demande_prix SET devis = 1, bcmd = 0, facture = 0 WHERE id_d = :id");
  $stmt->execute(['id' => $id]);
}

if(isset($_GET['bon'])) {
  $id = intval($_GET['id']);
  $stmt = $dbh->prepare("UPDATE demande_prix SET devis = 0, bcmd = 1, facture = 0 WHERE id_d = :id");
  $stmt->execute(['id' => $id]);
  $stmt2 = $dbh->prepare("INSERT INTO rest_payer(id_d,fornis,devis,bcmd,facture,payer,besoin,montant,date)
                        SELECT id_d, idfour, devis, bcmd, facture, payer, besoin, montant, date
                        FROM demande_prix
                        WHERE id_d = :id");
  $stmt2->execute(['id' => $id]);
...
if(isset($_GET['del'])){
  mysqli_query($con,"delete from demande_prix where id_d = '".$_GET['id']."'");
  mysqli_query($con,"delete from rest_payer where id_d = '".$_GET['id']."'");
    ...
    <div class="content-wrapper">
      ...
      <div class="card-header">
        <form method="post"  >
          ...
          <button  style="color:blue" class="btn"  type="submit" name="chercher" id="ch" ><i class="fa fa-search" ></i> Chercher</button>
          <button  style="color:blue" class="btn"  type="submit" name="cht" id="ch1" ><i class="fa fa-search" ></i> afficher tous</button>
          <div class="form-group col-md-3">
            <script>
        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Gérer les Demande de prix</h3>
                ...
                <div class="card-body mt-2 " >
                    ...
                    <tbody>
                      <?php 
                      $query=mysqli_query($con,"SELECT demande_prix.*, fournisseur.raison_sociale FROM demande_prix INNER JOIN fournisseur ON fournisseur.idfour = demande_prix.idfour  and DATE_FORMAT(date,'%Y-%m') >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 MONTH),'%Y-%m');");
                        if(isset($_POST['chercher'])){
                          $dd=$_POST['dd'];
                          $ff=$_POST['df'];
                          $query=mysqli_query($con,"SELECT demande_prix.*, fournisseur.raison_sociale FROM demande_prix INNER JOIN fournisseur ON fournisseur.idfour = demande_prix.idfour  and date between '$dd' and '$ff'");    
                        }
                        if(isset($_POST['cht'])) {
                          $query=mysqli_query($con,"SELECT demande_prix.*, fournisseur.raison_sociale FROM demande_prix INNER JOIN fournisseur ON fournisseur.idfour = demande_prix.idfour ;");
                        } 
                      $cnt=1;
                      while($row=mysqli_fetch_array($query))
                      {
                        ...
                        ?>
                        <tr>
                          <td class="donnees-td"><?php echo htmlentities($row['id_d']);?></td>
                          <td><a href="javascript:void(0);" onclick="submite(<?php echo $row['id_d'];?>);"><?php echo htmlentities($row['raison_sociale']);?></a></td>
                          <td <?php if(isset($_GET['mnt'])){echo "style=color:red;";}?>><?php echo htmlentities($row['montant']);?></td>
                          <form action="" method="post">

                            <td  <?php echo $style1; ?>>
                              <span style="color: transparent;"><?php echo htmlentities($row['devis']); ?></span>
                              <a href="list_demande_prix.php?id=<?php echo $row['id_d']?>&dev=delete" onClick="return confirm('Etes-vous sûr que vous voulez Modifier?')" style="color: black;" >
                                Devis</a>
                            </td>
                            <td <?php echo $style2; ?>>
                              <span style="color: transparent;"><?php echo htmlentities($row['bcmd']); ?></span>
                              <a href="list_demande_prix.php?id=<?php echo $row['id_d']?>&bon=delete" onClick="return confirm('Etes-vous sûr que vous voulez Modifier?')" style="color: black;">
                              B.commande</a>
                            </td>
                          </form>
                          ...
                          <td class="text-center">
                            ...
                            <a href="list_demande_prix.php?id=<?php echo $row['id_d']?>&del=delete" onClick="return confirm('Etes-vous sûr que vous voulez supprimer?')" class=" btn btn-danger btn-xs ">Supprimer</a>
                          </td>
                          <input type="hidden" name="id" value="<?php echo htmlentities($row['id_d']); ?>">
                        </tr>
                        <?php $cnt=$cnt+1;
                      } ?>
                    </tbody>
        ...
        filteredData.each(function (valueArray) {
            ...
            valueArray.forEach(function (value, index) {
                if (index !== 0 && index !== 6 && index !== 7) {
                    ...
                    cleanedValue = cleanedValue.replace("Devise", "").replace("Facture", "").trim();
...
const response = await fetch('getData.php?id_d=' + id_d);
