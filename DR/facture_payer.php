...
try {
    ...
    // Requête d'insertion
    $insertSql = "INSERT INTO facture_payer (id_f, idfour, facture, besoin, montant)
                SELECT id_d, fornis, facture, besoin, montant
                FROM rest_payer
                WHERE payer = 1";

    // Exécution de la requête d'insertion
    $rowsInserted = $dbh->exec($insertSql);
    ...
    $deleteSql = "DELETE FROM rest_payer WHERE payer = 1";

    // Exécution de la requête de suppression
    $rowsDeleted = $dbh->exec($deleteSql);
...
if(isset($_GET['del']))
{
    mysqli_query($con,"delete from facture_payer where id_f = '".$_GET['id']."'");
}
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1>Détails Facture payer</h1>
                        </div>
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="dash_dr.php">Accueil</a></li>
                                <li class="breadcrumb-item active">Gérer les Factures payer</li>
                            </ol>
                        </div>
                    </div>
                </div><!-- /.container-fluid -->
                                    ...
                                    <div class="modal-dialog modal-xl">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Détails Factures payer</h5>
                                ...
                                <div class="card-body mt-2 ">
                                    <div class="table-responsive">
                                        <table id="example1" class="table table-bordered table-hover">
                                            ...
                                            <tbody>
                                                <?php
                                                $query = mysqli_query($con, "SELECT facture_payer.*, fournisseur.raison_sociale FROM facture_payer INNER JOIN fournisseur ON fournisseur.idfour = facture_payer.idfour and DATE_FORMAT(date_payment,'%Y-%m') >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 MONTH),'%Y-%m');");
                                                if (isset($_POST['chercher'])) {
                                                    $dd = $_POST['dd'];
                                                    $ff = $_POST['df'];
                                                    $query = mysqli_query($con, "SELECT facture_payer.*, fournisseur.raison_sociale FROM facture_payer INNER JOIN fournisseur ON fournisseur.idfour = facture_payer.idfour  and date_payment between '$dd' and '$ff'");
                                                }
                                                if (isset($_POST['cht'])) {
                                                    $query = mysqli_query($con, "SELECT facture_payer.*, fournisseur.raison_sociale FROM facture_payer INNER JOIN fournisseur ON fournisseur.idfour = facture_payer.idfour ;");
                                                }
                                                $cnt = 1;
                                                ...
                                                ?>
                                                    <tr>
                                                        <td class="donnees-td"><?php echo htmlentities($row['id_f']); ?></td>
                                                        
                                                        <td><a href="javascript:void(0);" onclick="submite(<?php echo $row['id_f']; ?>);"><?php echo htmlentities($row['raison_sociale']); ?></a></td>
                                                        <td><?php echo htmlentities($row['montant']); ?></td>
                                                        <form action="" method="post">

                                                            <!-- <td <?php //echo $style1; ?>>
                                                                <span style="color: transparent;"><?php //echo htmlentities($row['devis']); ?></span>
                                                                <a href="list_demande_prix.php?id=<?php //echo $row['id_f'] ?>&dev=delete" onClick="return confirm('Etes-vous sûr que vous voulez Modifier?')" style="color: black;">
                                                                    Devis</a>
                                                            </td>

                                                            <td <?php //echo $style2; ?>>
                                                                <span style="color: transparent;"><?php //echo htmlentities($row['bcmd']); ?></span>
                                                                <a href="list_demande_prix.php?id=<?php //echo $row['id_f'] ?>&bon=delete" onClick="return confirm('Etes-vous sûr que vous voulez Modifier?')" style="color: black;">
                                                                    B.commande</a>
                                                            </td> -->
                                                            <td>Facture</td>

                                                        </form>
                                                        <td><?php echo htmlentities($row['date_payment']); ?></td>

                                                        <td>
                                                            <center>
                                                                <!-- <a href="telecharger_demande.php?path=<?php //echo htmlentities($row['besoin']);
                                                                                                            ?>"><button class="btn btn-warning text-light btn-xs text-center">Telecharger</button></a> -->
                                                                <a href="viewPdf.php?path=<?php echo htmlentities($row['besoin']); ?>"><button class="btn btn-warning text-light btn-xs text-center">Telecharger</button></a>
                                                            </center>
                                                        </td>
                                                        <td class="text-center">
                                                            <!-- <button class=" btn btn-primary btn-xs edit_data" id="<?php //echo  $row['id_f']; ?>" title="click for edit">Modifier</i></button> -->
                                                            <!-- <button class=" btn btn-success btn-xs edit_data2" id="<?php //echo  $row['id_f']; ?>" title="click for edit">Voir</i></button> -->
                                                            <a href="facture_payer.php?id=<?php //echo $row['id_f'] ?>&del=delete" onClick="return confirm('Etes-vous sûr que vous voulez supprimer?')" class=" btn btn-danger btn-xs ">Supprimer</a>
                                                        </td>
                                                        <input type="hidden" name="id" value="<?php echo htmlentities($row['id_f']); ?>">
                                                    </tr>
                                                <?php $cnt = $cnt + 1; } ?>
                                            </tbody>
                                        </table>
    ...
    <script type="text/javascript">
        function expo(){
            ...
            table.columns().every(function () {
                ...
                if(this.header().textContent !="#" && this.header().textContent !="Besoin" && this.header().textContent !="Action"){
