...
df.value = today;
                ...
                <div class="card-body mt-2 " >
                  ...
                  <table id="example1" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?php
$query = mysqli_query($con, "SELECT * FROM demande_prix_complex WHERE DATE_FORMAT(dated,'%Y-%m') >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 12 MONTH),'%Y-%m') and idcomplex=(select idcomplex from complexe1 where id=$_SESSION[sid]) and  payer=1");
if(isset($_POST['cht'])){
                        $query=mysqli_query($con,"SELECT * FROM demande_prix_complex where idcomplex=(select idcomplex from complexe1 where id=$_SESSION[sid]) and  payer=1");
                      }
                      if(isset($_POST['chercher'])){
                        $dd=$_POST['dd'];
                        $ff=$_POST['df'];
                        $query=mysqli_query($con,"SELECT * FROM demande_prix_complex WHERE dated between '$dd' AND '$ff' and idcomplex=(select idcomplex from complexe1 where id=$_SESSION[sid]) and  payer=1");    
                }
                      $cnt=1;
    ...
    <td>
    <a class="btn w-100  btn-sm disabled btn-danger"><small>Facture payée</small></a>
    </td>
                          <td><?php echo htmlentities($row['montant']);?></td>
                          <td><?php echo htmlentities($row['dated']);?></td>
                          <td>
                            <center>
                            <a class="btn btn-primary btn-xs" href="javascript:void(0);" onclick="submite(<?php echo htmlentities($row['id_demande']);?>);">Telecharger</a>
                            </center>
                          </td>
                        </tr>
                        <?php $cnt=$cnt+1;
...
table.columns().every(function () {
     ...
     if(this.header().textContent !="#" && this.header().textContent !="Action" && this.header().textContent !="Besoin"){
...
workbook.xlsx.writeBuffer().then(function (buffer) {
    ...
    a.href = url;
    a.download = 'factures payées.xlsx';
