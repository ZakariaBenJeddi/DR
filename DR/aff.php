...
<body class="hold-transition sidebar-mini">
  ...
  <div class="wrapper">
          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">TCD-AP</li>
              </ol>
            </div>
          </div>
                  ...
                  <table id="example1" class="table table-bordered table-hover" >
                    <thead> 
                      <tr> 
						...
						<th>Mode</th>
						<th>Groupe</th>
						<th>Somme de MH Totale  DRIF REC</th>
						<th>Somme de MH Réalisée Globale</th>
						<th>%</th>
   ...
   $query=mysqli_query($con,"select Niveau,groupe,CodeEFP,EFP,Creneau,codef,Complexe,MHRG,f,annee,Mode,sum(MHTTR) AS s,sum(MHRG) as g from drh group by groupe, CodeEFP;");
                      $cnt=1;
                      while($row=mysqli_fetch_array($query))
                      {
						  $p=($row['g']/$row['s'])*100;
						  $position = strpos($p,".");
						  $s1 = substr($p,0,$position);
						  ...
						  <td><?php echo htmlentities($row['Mode']);?></td>
							 <td><?php echo htmlentities($row['groupe']);?></td>
							  ...
							  <td><?php echo htmlentities($row['g']);?></td>
							   <td><?php echo htmlentities($s1).'%';?></td>
...
@include("includes/foot.php"); ?>
 <script src="https://cdnjs.cloudflare.com/ajax/libs/exceljs/4.3.0/exceljs.min.js"></script>
  ...
  function expo(){
    ...
    var worksheet = workbook.addWorksheet('TCD-AP2');
